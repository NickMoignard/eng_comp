/* 	~ All significant variables are set using #define
	~ The table is two-dimensional, so the loop structure is
		also two dimensional
	~ The inner loop is there simply to calculate one value, and is not part of the tables structure
	~ Inline comments and blank lines are used to seperate the main components of the program
